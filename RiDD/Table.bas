...
Public Sub AddToParent(Act As Activity, Left As Int, Top As Int, Width As Int, Height As Int)
	visible = True
	Header.Initialize("")
...
Public Sub GetValue(Col As Int, Row As Int)
	...
	Dim values() As String
	values = Data.Get(Row)
	Dim mUser As ManageUsers = Starter.ManageUsers1
	mUser.lblUserDataN.Text = "First Name: " & values(0)
	mUser.lblUserDataL.Text = "Last Name: " & values(1)
	mUser.lblUserDataA.Text = "Age: " & values(2)
	mUser.lblUserDataS.Text = "Status: " & values(3)
	'mUser.lblUserDataI.Text = "IDD Detected: " & values(4)
	'Starter.wsh.Querry("select date,idd from riddUserD","QUERRYLatestIdd")

	mUser.summaryReport.Text = "The " & values(3) & " user named " & values(0) & " " & values(1) & ", " & values(2) & " years of age has a possibility of having the following on the table below." '& values(4)
	Dim ridd_qr As riddQR = Starter.riddQR1
	'Try
		'If values(0).Length<>0 And values(1).Length<>0 Then
			'Msgbox("You selected an empty cell.", "Select again")
		'Else
			ridd_qr.lblQRNum.Text = values(0) & " " & values(1)
		'End If
	'Catch
		'Log(LastException)
		'Msgbox("You selected an empty cell.", "Select again")
...
Public Sub LoadTableFromCSV(Dir As String, Filename As String, HeadersExist As Boolean)
	...
	Next
End Sub
public Sub SetDatagrid(HeaderX() As String,x As List)
	
	innerClearAll(HeaderX.Length)
	ColumnWidth = SV.Width / NumberOfColumns
	SetHeader(HeaderX)
	For Each a In x
		AddRow(a)
